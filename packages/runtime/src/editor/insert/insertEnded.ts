...
export async function insertEnded(insertState: InsertState, canceled: boolean) {
  ...
  await tryStartViewTransition(() => {
    ...
    insertState.repeatedNodes.toReversed().forEach((node) => {
      ...
      node.classList.remove('insert-repeat-node')
      node.style.removeProperty('rotate')
      node.style.removeProperty('--insert-repeat-node-width')
      node.style.removeProperty('--insert-repeat-node-height')
      node.style.removeProperty('--insert-repeat-node-translate')
      node.style.removeProperty('--insert-repeat-node-rotate')
      node.style.removeProperty('--insert-repeat-node-opacity')
    })
    removeDropHighlight()
  }).finished
