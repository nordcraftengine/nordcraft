...
>(key: TKey, value: TValue, message?: TMessage) {
  const base = v.record(key, value, message)
  const run = base['~run']
  ;(base as { '~run': unknown })['~run'] = function (
    dataset: v.UnknownDataset,
    config: v.Config<v.BaseIssue<unknown>>,
  ) {
    if (Array.isArray(dataset.value)) {
      v._addIssue(
        this as unknown as Parameters<typeof v._addIssue>[0],
        'type',
        dataset,
        config,
        { expected: 'Object' },
    ...
    return run.call(this, dataset, config)
  }
  return base
}
